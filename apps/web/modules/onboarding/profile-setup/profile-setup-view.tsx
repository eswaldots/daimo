...
const schema = z.object({
  ...
  age: z
    .number({ error: "La edad tiene que ser un número" })
    .min(1, { error: "La edad es requerida" }),
  gender: z.union([
    z.literal("niño", { error: "El genero es requerido" }),
    z.literal("niña"),
  ]),
});
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        <div className="gap-4 grid text-center">
          <StepTitle>Háblanos sobre tu hijo/a</StepTitle>
          <StepDescription>
            Ayúdanos a personalizar su experiencia dándonos detalles básicos de
            tu hijo/a
          </StepDescription>
        </div>
        <motion.div
        ...
        >
          <FieldGroup className="gap-1 w-full">
            ...
            <Label className="mt-3 font-normal text-muted-foreground">
            ...
            <div className="mb-1 mt-2 flex flex-row justify-center gap-4 w-full">
              <div
                ...
                className={cn(
                  "cursor-pointer hover:bg-secondary/80 transition-colors peer-data-[state='checked']:bg-black flex flex-col items-center relative justify-center gap-3 rounded-xl py-6 flex-1 bg-secondary",
              ...
              >
                ...
                <Label className="text-sm pointer-events-none">Niña</Label>
              </div>
            </div>
          ...
          <div className="my-10">
            <DataPrivacy>
              <FieldDescription className="flex items-center gap-2 my-4 text-sm hover:underline cursor-pointer text-accent">
                <LockIcon className="size-4" />
                ¿Por que recolectamos estos datos?
              </FieldDescription>
            </DataPrivacy>
            <Button
              variant="accent"
              className="py-6 text-base rounded-full w-full"
              size="lg"
            >
      ...
      <DialogContent className="md:max-w-2xl w-full p-10 md:p-16 overflow-y-scroll max-h-[80vh]">
        ...
        <DialogFooter>
          <DialogClose asChild>
            <Button
              size="lg"
              className="ml-auto w-fit text-base rounded-full mt-8"
            >
              Entendido
            </Button>
          </DialogClose>
...
const FormSelect = ({ placeholder, ...props }: ComponentProps<"input">) => {
  ...
  return (
    <Select onValueChange={setValue}>
      <SelectTrigger
        className="w-full py-7.5 bg-secondary relative px-3 rounded-xl"
        onBlur={() => {
          if (value.length >= 1) return;
