...
export const createApiKey = internalAction({
		...
		handler: async (ctx, args) => {
				const key = generateApiKey(args.name.length || 16);

				 await ctx.runMutation(internal.auth.apiKey.createMutation, { ...args, key });
...
export const verify = internalQuery({
		...
		handler: async (ctx, { key }) => {
				const apiKey = await ctx.db.query("apiKeys").withIndex("by_key", q => q.eq("key", key)).first();
...
export const verifyApiKey = async (ctx: GenericQueryCtx<DataModel>, key: string) => {
